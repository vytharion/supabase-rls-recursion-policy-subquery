...
def test_step5_policy_reads_from_auth_jwt() -> None:
    ...
    # The headline property of fix attempt 2: the policy decides from
    # the token, not from a database lookup. auth.jwt() is the
    # documented accessor for the request's verified claim set.
    assert re.search(r"auth\.jwt\s*\(\s*\)", clause)


def test_step5_policy_references_app_metadata_claim() -> None:
    clause = _policy_clause(_fix_sql())
    # app_metadata is the server-controlled half of the JWT — the
    # client cannot mutate it. user_metadata would be wrong here
    # because a malicious caller could rewrite that from the browser.
...
def test_step5_policy_uses_jsonb_containment_operator() -> None:
    ...
    assert re.search(r"\?\s*(?:public\.)?memberships\.team_id::text", clause) or \
        re.search(r"\?\s*memberships\.team_id::text", clause) or \
        re.search(r"\?\s*team_id::text", clause)
...
def test_step5_policy_has_fallback_for_missing_claim() -> None:
    ...
    assert re.search(r"'\[\]'\s*::\s*jsonb", clause) or re.search(
        r"'\[\]'::jsonb", clause
    )
