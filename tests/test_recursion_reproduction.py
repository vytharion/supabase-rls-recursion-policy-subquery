"""Structural tests for the step-3 recursion-reproduction artifacts.

Step 3 ships two side-car files next to the migration: a SQL script that
...
``docs/03-*.mdx`` can quote them verbatim and stay in sync with the code.

The matching trace artefacts — ``recursion_trace.sql`` and
``recursion_trace_notes.md`` — are exercised in ``test_recursion_trace.py``
so the two halves of step 3 (reproduce + trace) stay testable in
isolation.
"""
